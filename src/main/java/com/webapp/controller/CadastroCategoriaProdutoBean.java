...
public class CadastroCategoriaProdutoBean implements Serializable {
	...
	public void prepararEditarCadastro() {
		categoriaProduto = categoriasProdutos.porId(categoriaProdutoSelecionado.getId());
	}
	...
	public void salvar() {
		...
		if (categoriaProduto.getId() == null) {
			...
			CategoriaProduto categoriaProdutoTemp = categoriasProdutos.porNome(categoriaProduto.getNome(), usuario.getEmpresa());
		
			if(categoriaProdutoTemp == null) {
				...
				PrimeFaces.current().executeScript(
						"PF('downloadLoading').hide();PF('categoriaProduto-dialog').hide();swal({ type: 'success', title: 'Concluído!', text: 'Categoria de produto salvo com sucesso!' });");
				...
				PrimeFaces.current().executeScript(
						"swal({ type: 'error', title: 'Erro!', text: 'Já existe uma categoria de produto com o nome informado!' });");
			}
		
		} else {
			
			CategoriaProduto categoriaProduto = categoriasProdutos.porNomeCadastrado(this.categoriaProduto, usuario.getEmpresa());
			
			if(categoriaProduto == null) {
				
				this.categoriaProduto.setEmpresa(usuario.getEmpresa());
				
				categoriasProdutos.save(this.categoriaProduto);
				...
				PrimeFaces.current().executeScript(
						"PF('downloadLoading').hide();PF('categoriaProduto-dialog').hide(); swal({ type: 'success', title: 'Concluído!', text: 'Categoria de produto atualizada com sucesso!' });");
