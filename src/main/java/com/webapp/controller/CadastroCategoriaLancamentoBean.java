...
public class CadastroCategoriaLancamentoBean implements Serializable {
	...
	public void prepararEditarCadastro() {
		categoriaDespesa = categoriasDespesas.porId(categoriaDespesaSelecionada.getId());
	}
	...
	public void salvar() {
		...
		if (categoriaDespesa.getId() == null) {
			...
			DestinoLancamento destinoLancamento = destinosLancamentos.porId(43L);
			categoriaDespesa.setEmpresa(usuario.getEmpresa());
			categoriaDespesa.setDestinoLancamento(destinoLancamento);
	
			CategoriaLancamento categoriaLancamentoTemp = categoriasDespesas.porNome(categoriaDespesa.getNome(), usuario.getEmpresa());
		
			if(categoriaLancamentoTemp == null) {
				...
				PrimeFaces.current().executeScript(
						"PF('downloadLoading').hide();PF('tipoDespesa-dialog').hide(); swal({ type: 'success', title: 'Concluído!', text: 'Categoria de lançamento salva com sucesso!' });");
				...
				PrimeFaces.current().executeScript(
						"swal({ type: 'error', title: 'Erro!', text: 'Já existe uma categoria de lançamento com o nome informado!' });");
			}
		
		} else {
			System.out.println(categoriaDespesa.getId());
			System.out.println(categoriaDespesa.getNome().toUpperCase());
			System.out.println(usuario.getEmpresa());
			CategoriaLancamento categoriaLancamentoTemp = categoriasDespesas.porNomeCadastrado(categoriaDespesa, usuario.getEmpresa());
			...
			if(categoriaLancamentoTemp == null) {
				...
				PrimeFaces.current().executeScript(
						"PF('downloadLoading').hide();PF('tipoDespesa-dialog').hide(); swal({ type: 'success', title: 'Concluído!', text: 'Categoria de lançamento atualizada com sucesso!' });");
