...
public class CategoriasLancamentos implements Serializable {
	...
	public CategoriaLancamento porNome(String nome, String empresa) {
		...
		try {
			categoriaLancamento = this.manager.createQuery("from CategoriaLancamento i where (i.empresa is null OR i.empresa = :empresa) AND upper(i.nome) like :nome order by nome", CategoriaLancamento.class)
					.setParameter("empresa", empresa).setParameter("nome", "%" + nome.toUpperCase() + "%").getSingleResult();
	...
	public CategoriaLancamento porNomeCadastrado(CategoriaLancamento categoriaLancamento, String empresa) {
		try {
			return this.manager  
					.createQuery("from CategoriaLancamento e where lower(e.nome) = :nome and e.id != :id and e.empresa = :empresa", CategoriaLancamento.class)
					.setParameter("nome", categoriaLancamento.getNome().toLowerCase())
					.setParameter("id", categoriaLancamento.getId())
